...
export BUILDER_IMAGE=ghcr.io/0conf/caph-builder
...
if
  docker manifest inspect ghcr.io/0conf/caph-builder:"${NEW_VERSION}" >/dev/null
  echo $?
then

  sed -i -e "/^BUILDER_IMAGE_VERSION /s/:=.*$/:= ${NEW_VERSION}/" Makefile
  grep -r -E 'ghcr.io/0conf/caph-builder:[0-9].*.*' -l | xargs sed -i -e "s/ghcr.io\/syself\/caph-builder:${VERSION}/ghcr.io\/syself\/caph-builder:${NEW_VERSION}/g"
  docker build -t ghcr.io/0conf/caph-builder:"${NEW_VERSION}" ./images/builder
  docker push ghcr.io/0conf/caph-builder:"${NEW_VERSION}"
else
  exit 1
fi
